...
// ---------------------------------------------------------------------
   ...
   */
   ...
   public static final double SphToEcl(double L, double B, double O) {
     double G; // Longitude value in Radians, also die L�nge und das ist genau der Winkel auf der Ekliptik
     ...
     Q = xy.y; // z-Koordinate merken, also die H�he der Kugel (nach Formeln Seite 264).
     ...
     xy = PolToRec(ra.x,ra.y+O); // um die Schiefe erh�hten Winken wieder zur�ckrechnen

     // Q = Math.asin(xy.y);
     ra = RecToPol(G, xy.x); // (x,y) wieder auf Polar, H�he ist wurscht, weil jetzt in Ekliptik bin
     ...
     // xy.x = G; // Longitude value in Radians, also die L�nge und das ist genau der Winkel auf der Ekliptik
...
// ---------------------------------------------------------------------
   ...
   */
   ...
   /** Berechnet Winkel zu Punkt (x,y), d.h. den ATAN(y/x), wobei Sonderf�lle,
...
// ---------------------------------------------------------------------
   ...
   /** Berechnet den Julianischen Tag aus dem angegebenen Tag, Monat, Jahr.
       ...
       (also -4712) um 12:00. Diese Routine ist gut f�r alle Datumsangaben seit Tag 0.
   ...
   */
   ...
   /* DEBUG/unused
   /** Berechnet den Julianischen Tag aus dem angegebenen Tag, Monat, Jahr.
       Diese Routine ist gut f�r Datumsangaben von 1.3.1800 bis 28.2.2100.
       @param day Tag
       @param month Monat
