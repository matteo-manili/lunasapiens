 ...
 * <p>
 * The idea behind this is in having an option to generate similar sources out
 * of one code base. So you can comment / uncomment different parts of the code
 * via precompile switches in the source files or as arguments to this class
 * (-D).
 * <p>
 * Syntax:<br>
 * <blockquote> <code>
...
* </code> </blockquote> In the first mode, you give this class at least one
 * directory as parameter, this is the directory, where the input source files
 * reside (-i). Optionally, you specify with the -o switch, where
 * Precompile.java should put the precompiled source files. If the output
 * directory does already exist, and it is not empty, Precompile will stop. Use
 * the -f switch, to force overwriting existing files in the output directory.
 * <br>
 ...
 * switches in all the source files only. Supported precompile switches are:<br>
 * <blockquote>
 * <ul>
 * <li><code>#define &lt;value&gt;</code><br>
 * <blockquote> Only to define a switch for evaluation by <code>#ifdef</code> or
 * <code>#ifndef</code>. You can also #define values from the command line by
 * the -D switch. Locally defined switches are restricted to the file in which
 * they occur. </blockquote>
 * <li><code>#undef &lt;value&gt;</code><br>
 * <li><code>#undefine &lt;value&gt;</code><br>
 * <blockquote> Only to undefine a previously set switch. #undefine is also
 * restricted to the file in which it appears. </blockquote>
 * <li><code>#if &lt;value&gt; ...  ... </code><br>
 * <blockquote> This can be <i></i> (==false) or <i>#if 1</i> (or any other
 * Integer != 0: ==true). Otherwise it is interpreted identical to <i>#ifdef
 * &lt;value&gt;</i>. </blockquote>
 * <li><code>#ifdef &lt;value&gt; ...  ... </code><br>
 * <blockquote> True, if &lt;value&gt; is defined prior to this line of code,
 * false otherwise. </blockquote>
 * <li><code>&lt;value&gt; ...  ... </code><br>
 * <blockquote> Equal to <code>#ifdef</code>, but with reversed
 * logic. </blockquote>
 * </ul>
 * </blockquote>
 */
public class Precompile {
	...
	static final String shortSyntax = "java Precompile [-h|-help|--help]\n"
			+ "java Precompile [-q] [-f] [-c] -i<InputDir> [-i<InputDir> ...]\n"
			+ "                [-o<OutputDir> [-o<OutputDir> ...]]\n" + "                {-D<definedParameter>...}\n"
			+ "java Precompile -l -i<InputDir> [-i<InputDir> ...]";
	 ...
	 * for the source files. Optionally, you can say, into which directory to
	 * put the precompiled sources. By default, Precompile.java will parse the
	 * input file for a package statement, and output the file to that
	 * directory.<br>
	 * 
	 * @param inputDirs
	 *            In which directories are the java source files? ALL Java
	 *            source files in these directories will be precompiled.
	 * @param outputDirs
	 *            In which directories to put the precompiled source files. They
	 *            do not have to exist, they will be created automatically if
	 *            needed.<br>
	 *            This parameter may be null or empty. In this case, the method
	 *            will try to determine the output directory by parsing the
	 *            input file for a package statement. This method is not
	 *            absolutely fool prove, but should work in most cases, where
	 *            the package statement is not wrapped up in some unusual
	 *            context.
	 * @param force
	 *            A boolean to force overwriting the contents of any existing
	 *            output directories before writing to them. If force is false,
	 *            and an output directory is not empty, a
	 *            GeneralSecurityException is thrown.
	 * @param switches
	 *            Any &lt;DEFINES&gt; to be used as precompile options. Can be
	 *            null if none should be applied.
	 * @return A String array with warnings. Each warning String consists of
	 *         three parts, first the filename, then the lineNumber and then the
	 *         error message. All three parts are separated by '\n'.
	 */
	public String[] precompile(String[] inputDirs, String[] outputDirs, boolean force, String[] switches)
		...
		if (ret != 0) {
			for (int dirs = 0; dirs < inDir.size(); dirs++) {
				// Writes a file list to the global arrays inputFiles and
				// outputFiles:
				getFiles(dirs);
				for (int i = 0; i < inputFiles.length; i++) {
					fname = inputFiles[i];
					lineCnt = 0;
					precompileFile(inputFiles[i], outputFiles[i]);
	 ...
	 * precompilation.
	 * <p>
	 * Parameters in detail: <blockquote>
	 * <ul>
	 ...
	 * directories in the same way as it is explained above for the -i switch.
	 * <br>
	 * If you don't give output directories, Precompile.java will try to find a
	 * "package" statement in the source file, and use the package name as the
	 * output directory for that file. The method of finding the package name
	 * should be rather robust, but in case it fails, this is, you don't find
	 * the resulting file in the correct directory, specify the output
	 * directories with the -o switch(es). Also, if you prefer to put the output
	 * files into another directory, than the package name implies, you have to
	 * use -o&nbsp;.
	 * <li><code>-q</code> (Optional) Quiet mode. Don't output anything except
	 * for errors.
	 ...
	 * When using Precompile.java without -o switch, you might first want to
	 * test without -f to see, if Precompile.java outputs to the directories you
	 * think. In following precompiles, you can then add the -f switch.
	 ...
	 * 
	 * @param params
	 *            An array of Strings. See above for a more detailed explanation
	 *            of possible parameters.
	 */
	public static void main(String[] params) {
		Precompile p = new Precompile();
		try {
			if (p.readParams(params) == 0) {
				System.exit(0);
			}
		...
		if (p.listDefines) {
			...
			for (int dirs = 0; dirs < p.inDir.size(); dirs++) {
				...
				for (int i = 0; i < p.inputFiles.length; i++) {
					...
					p.lineCnt = 0;
					p.precompileFile(p.inputFiles[i], p.outputFiles[i]);
	...
	int readParams(String[] p) {
		...
		int i = 0;
		for (; i < p.length; i++) {
			if (p[i].equals("-h") || p[i].equals("-help") || p[i].equals("--help")) {
				...
				if (p[i].length() > 2) {
					outDir.addElement(p[i].substring(2));
				} else if (i + 1 < p.length) {
				...
				if (p[i].length() > 2) {
					inDir.addElement(p[i].substring(2));
				} else if (i + 1 < p.length) {
		...
		if (err) {
			throw new IllegalArgumentException(
					"Unknown parameter or missing value for " + "parameter '" + p[i] + "'.\n" + shortSyntax);
		}
		if (listDefines && inDir.size() == 0) {
			throw new IllegalArgumentException("Missing parameter -i.\nSyntax:\n" + shortSyntax);
		}
		...
		if (!listDefines && inDir.size() != outDir.size() && outDir.size() != 0) {
			throw new IllegalArgumentException(
					"Number of input directories (" + inDir.size() + ") and " + "number of output directories ("
	...
	void getFiles(int index) {
		...
		if (outDir != null && outDir.size() > 0) {
			...
			if (inDir.equals(out)) {
				throw new IllegalArgumentException(
						"Input directory and output directory have to " + "be different ('" + inDir + "')!");
			}
		}
		...
		if (!listDefines && out != null) {
			...
			if (fOutDir.exists() && !fOutDir.isDirectory()) {
				throw new IllegalArgumentException("Cannot create directory '" + out + "'.");
		...
		if (inputFiles.length < 1) {
			throw new IllegalArgumentException("No source files (\"" + inDir + File.separator + "*.java\") found.");
		}
	...
	String getPackageDir(String file) {
		...
		try {
			...
			while ((line = fIn.readLine()) != null) {
				lineCnt++;
				// We don't have to care about comment strings in string
				// constants in
				// the moment, as the package statement has to occur before any
				// string
	...
	void precompileFile(String fNameIn, String fNameOut) {
		...
		// Initialize the #defines to the state of calling the precompile
		// routine:
			...
			while ((line = fIn.readLine()) != null) {
				...
				if (line.startsWith("#")) {
					...
					if (cmd.startsWith("#if")) {
						...
						if (max < 0) {
							vWarnings.addElement(fname + "\n" + lineCnt + "\n found "
									+ "without previous #if or #ifdef or " + "#ifndef.");
						} else {
						...
						try {
							...
							vWarnings.addElement(fname + "\n" + lineCnt + "\n#undefine found " + "without any value.");
						}
					} else if (write.booleanValue()) {
						vWarnings.addElement(fname + "\n" + lineCnt + "\nUnknown directive '" + cmd + "' found.");
					}
				} else if (write.booleanValue()) {
					fOut.write(line + "\n");
				}
		...
		if (miss > 0) {
			vWarnings.addElement(fname + "\n" + lineCnt + "\nMissing " + miss + " '' " + "statement"
					+ (miss > 1 ? "s" : "") + " on end of file!");
	...
	boolean evaluate(String cmd, StringTokenizer tk, boolean w) {
		...
		if (cmd.equals("#ifdef") || cmd.equals("#if")) { // The logic for #if is
															// not identical to
															// the
															// logic of #ifdef
															// in C, but here
